...
const policyHours = computed(
  () => response.value?.cancellation_policy_hours ?? 24,
);
...
async function loadSlots() {
  ...
  try {
    ...
    if (
      form.hora_inicio &&
      !available.some((s) => s.time === form.hora_inicio)
    ) {
      form.hora_inicio = "";
    }
  } catch {
...
onMounted(() => {
  const barberId =
    typeof route.query.barber === "string" ? route.query.barber : "";
...
async function ensureStripeMounted() {
  ...
  cardElement.on("change", ({ error: elError }) => {
    payError.value = elError?.message ?? "";
  });
}
  ...
  <div class="p-4 sm:p-6 lg:p-8">
    ...
    >
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold text-ink">
          Mis <span class="text-gold">Citas</span>
        </h1>
        <p class="mt-1 text-sm text-muted">
          Puedes reagendar o cancelar hasta {{ policyHours }} horas antes.
        </p>
      </div>
    ...
    >
      <div class="ui-card p-4">
        ...
        <p class="mt-1 text-xl font-black text-ink">{{ stats.total }}</p>
      </div>
      <div class="ui-card p-4">
        <p class="text-[10px] font-bold uppercase text-muted">Próximas</p>
        <p class="mt-1 text-xl font-black text-gold">{{ stats.proximas }}</p>
      </div>
      <div class="ui-card p-4">
        <p class="text-[10px] font-bold uppercase text-muted">Completadas</p>
        <p class="mt-1 text-xl font-black text-emerald-400">
          {{ stats.completadas }}
        </p>
      </div>
      <div class="ui-card p-4">
        <p class="text-[10px] font-bold uppercase text-muted">Canceladas</p>
    ...
    <section
      v-if="next"
      class="mb-6 rounded-2xl border border-gold/30 bg-gold/5 p-5"
    >
      <p class="text-[10px] font-bold uppercase tracking-widest text-gold">
        Tu próxima cita
      </p>
      <p class="mt-1 font-black text-ink">
        {{ next.service.nombre }} con {{ next.barber.user.name }}
      </p>
      <p class="text-sm text-muted">
        {{ fmtDate(next.fecha) }} · {{ next.hora_inicio?.slice(0, 5) }}
      </p>
    </section>
    ...
    <p v-if="cancelError" class="mb-3 text-sm text-red-400">
      {{ cancelError }}
    </p>
      ...
      <div v-for="appt in appointments" :key="appt.id" class="ui-card p-5">
        <div class="mb-2 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p class="text-sm text-muted">
              {{ fmtDate(appt.fecha) }} ·
              {{ appt.hora_inicio?.slice(0, 5) }} con
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <template v-if="canManage(appt)">
            <button
              type="button"
              class="rounded-lg border border-line px-3 py-1.5 text-xs text-muted hover:text-ink"
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                v-model="form.hora_inicio"
                required
                :disabled="slotsPending || !slots.length"
    ...
    >
      <div class="w-full max-w-sm rounded-2xl border border-line bg-card p-6">
        ...
        <template v-if="!paySucceeded">
          ...
          />
          <p v-if="payError" class="mt-2 text-sm text-red-400">
            {{ payError }}
          </p>
          <p class="mt-2 text-[11px] text-muted">
            Beta: el monto se calcula del lado del servidor, nunca se envía
            desde aquí.
          </p>
          <div class="mt-5 flex gap-3">
            <button
        ...
        <template v-else>
          <div
            class="rounded-lg border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-300"
          >
            Pago recibido.
            {{ payConfirming ? "Confirmando con el sistema…" : "Listo." }}
          </div>
          <button
            type="button"
            class="mt-4 w-full rounded-lg border border-line px-4 py-2 text-sm text-muted hover:text-ink"
            @click="closePay"
          >
            Cerrar
          </button>
