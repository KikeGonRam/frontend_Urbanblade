...
const SEGMENT_LABEL: Record<string, string> = { vip: 'VIP', new: 'Nuevo', active: 'Activo', inactive: 'Inactivo' }
...
const { data: response, pending, error, refresh } = await useAsyncData<ClientsResponse>(
  'clients-list',
...
const { data: segmentation } = await useAsyncData('clients-segmentation', () =>
  apiFetch<{ success: boolean, data: Record<string, { count: number, percentage: number }> }>('/admin/clients/segmentation/data'),
)
  ...
  try {
    if (editing.value) {
      ...
      await apiFetch('/admin/clients', {
        method: 'POST',
        body: { name: form.name, email: form.email, telefono: form.telefono, password: form.password },
      })
    }
  ...
  <div class="p-4 sm:p-6 lg:p-8">
    <header class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold text-ink">Gestión de <span class="text-gold">Clientes</span></h1>
        <p class="mt-1 text-sm text-muted">Base de datos completa de clientes registrados.</p>
      </div>
    ...
    <section class="mb-5 grid grid-cols-2 gap-4 sm:grid-cols-4">
      <button
        ...
        :class="segment === card.key ? 'border-gold/40 bg-gold/5' : 'border-line bg-card hover:border-gold-dim'"
        @click="segment = segment === card.key ? '' : card.key; resetAndSearch()"
      >
        <p class="mb-1 text-[10px] font-black uppercase tracking-wider text-muted">{{ card.label }}</p>
    ...
    <section class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      >
      <button v-if="search || segment" type="button" class="text-xs font-bold uppercase tracking-widest text-muted hover:text-ink" @click="search = ''; segment = ''; resetAndSearch()">
        Limpiar filtros
      </button>
    </section>
    ...
    <section v-else class="ui-card overflow-x-auto">
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr v-for="client in clients" :key="client.id" class="border-b border-line/60 last:border-0">
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                ...
                <span class="font-bold text-ink">{{ client.name ?? 'Sin usuario' }}</span>
              </div>
            </td>
            <td class="px-4 py-3 text-muted">{{ client.email ?? '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ client.telefono ?? '—' }}</td>
            <td class="px-4 py-3 text-center">
              <span class="rounded-full border px-2 py-0.5 text-[10px] font-black" :class="SEGMENT_CLASS[client.segment]">{{ SEGMENT_LABEL[client.segment] }}</span>
            </td>
            <td class="px-4 py-3 text-center text-ink">{{ client.totalAppointments }}</td>
            <td class="px-4 py-3 text-muted">{{ fmtDate(client.joinedAt) }}</td>
            <td class="px-4 py-3">
              <div v-if="client.slug" class="flex justify-end gap-2">
                <button type="button" class="rounded-lg border border-line px-3 py-1 text-xs text-muted hover:text-ink" @click="openEdit(client)">Editar</button>
                <button type="button" class="rounded-lg border border-red-500/20 px-3 py-1 text-xs text-red-400 hover:bg-red-500/10" @click="removeClient(client)">Eliminar</button>
              </div>
            </td>
          </tr>
          <tr v-if="!clients.length">
            <td colspan="7" class="px-4 py-12 text-center text-sm text-muted">Sin clientes que mostrar.</td>
          </tr>
        </tbody>
      </table>
    </section>
    ...
    <div v-if="lastPage > 1" class="mt-4 flex items-center justify-center gap-3">
      <button type="button" class="rounded-lg border border-line px-3 py-1.5 text-sm text-ink disabled:opacity-40" :disabled="page <= 1" @click="page--">←</button>
      <span class="text-sm text-muted">Página {{ page }} de {{ lastPage }}</span>
