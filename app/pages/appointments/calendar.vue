...
interface CalendarEvent {
  ...
  color: string
  textColor: string
  extendedProps: { cliente: string, servicio: string, barbero: string, estado: string }
}
...
onMounted(async () => {
  try {
    const res = await apiFetch<{ data: Array<{ id: string, user: { name: string } | null }> }>('/barbers')
    barbers.value = res.data.map((b) => ({ id: b.id, name: b.user?.name ?? 'Barbero' }))
  } catch {
    barbers.value = []
  }
})
...
async function fetchEvents(
  ...
  try {
    const events = await apiFetch<CalendarEvent[]>('/appointments/calendar-data', {
      query: { start: info.startStr, end: info.endStr, barber_id: selectedBarber.value || undefined },
    })
  ...
  <div class="p-4 sm:p-6 lg:p-8">
    <header class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold text-ink">Calendario de <span class="text-gold">Citas</span></h1>
        <p class="mt-1 text-sm text-muted">Vista mensual, semanal y diaria de la agenda.</p>
      </div>
    </header>
    ...
    <div class="space-y-5">
      <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div class="flex flex-wrap gap-3">
          <span v-for="item in legend" :key="item.label" class="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-muted">
        ...
        <div class="flex items-center gap-3">
          ...
          <select v-model="selectedBarber" class="min-w-[180px] rounded-lg border border-line bg-main px-3 py-2 text-sm text-ink" @change="refetchEvents">
      ...
      <div v-if="modal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" @click.self="modal.open = false">
        <div class="relative w-full max-w-md rounded-3xl border border-line bg-card p-8 shadow-[0_30px_80px_rgba(0,0,0,0.7)]">
          <button aria-label="Cerrar" class="absolute right-4 top-4 text-muted transition hover:text-ink" @click="modal.open = false">
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          <div class="absolute inset-x-0 top-0 h-px rounded-t-3xl bg-gradient-to-r from-transparent via-gold/30 to-transparent" />

          <div class="mb-4 h-3 w-3 rounded-full" :style="{ background: modal.color }" />
          <h3 class="mb-1 text-xl font-black uppercase tracking-tight text-ink">{{ modal.title }}</h3>
          <p class="mb-6 text-[10px] font-bold uppercase tracking-widest text-muted">{{ modal.time }}</p>

          <div class="space-y-4">
            <div class="flex items-center gap-3">
              <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gold/10 text-gold">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
              </div>
              <div>
                <p class="text-[9px] font-bold uppercase tracking-widest text-muted">Cliente</p>
            ...
            <div class="flex items-center gap-3">
              <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gold/10 text-gold">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M6 9a3 3 0 100-6 3 3 0 000 6zm0 12a3 3 0 100-6 3 3 0 000 6zm14-15L8.5 15M9 6l11 12" />
                </svg>
              </div>
              <div>
                <p class="text-[9px] font-bold uppercase tracking-widest text-muted">Servicio</p>
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-[9px] font-bold uppercase tracking-widest text-muted">Barbero</p>
