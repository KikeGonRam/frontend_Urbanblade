...
const { data: barbersRes } = await useAsyncData('appointments-barbers', () =>
...
const { data: servicesRes } = await useAsyncData('appointments-services', () =>
...
watch(clientQuery, (q) => {
  ...
  clientSearchTimer = setTimeout(async () => {
    try {
      const res = await apiFetch<{ data: Array<{ id: string, user: { name: string, email: string } | null }> }>('/clients', { query: { q } })
      clientOptions.value = res.data.map((c) => ({ id: c.id, name: c.user?.name ?? 'Cliente', email: c.user?.email ?? '' }))
    } catch {
      clientOptions.value = []
    }
  }, 350)
  ...
  try {
    if (editing.value) {
      ...
      await apiFetch(`/appointments/${editing.value.code}`, {
        method: 'PUT',
        body: {
          client_id: form.client_id, barber_id: form.barber_id, service_id: form.service_id,
          fecha: form.fecha, hora_inicio: form.hora_inicio, estado: form.estado, notas: form.notas || undefined,
        },
      })
    } else {
...
async function cancelAppointment(appt: AppointmentRow) {
  ...
  try {
    await apiFetch(`/appointments/${appt.code}`, {
      method: 'PUT',
      body: {
        client_id: appt.client.id, barber_id: appt.barber.id, service_id: appt.service.id,
        fecha: appt.fecha, hora_inicio: appt.hora_inicio?.slice(0, 5), estado: 'cancelada', notas: appt.notas ?? undefined,
      },
    })
    await refresh()
  } catch (err: unknown) {
  ...
  <div class="p-4 sm:p-6 lg:p-8">
    <header class="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold text-ink">Citas & <span class="text-gold">Reservas</span></h1>
        <p class="mt-1 text-sm text-muted">Gestiona la agenda completa de la barbería.</p>
      </div>
    ...
    <section class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <button v-if="estadoFilter || barberFilter" type="button" class="text-xs font-bold uppercase tracking-widest text-muted hover:text-ink" @click="estadoFilter = ''; barberFilter = ''">
        Limpiar filtros
      </button>
      <span class="text-xs text-muted sm:ml-auto">Mostrando hasta 50 citas más recientes.</span>
    </section>
    ...
    <section v-else class="ui-card overflow-x-auto">
      <table class="w-full text-left text-sm">
        ...
        <tbody>
          <tr v-for="appt in appointments" :key="appt.id" class="border-b border-line/60 last:border-0">
            <td class="px-4 py-3 text-ink">{{ appt.fecha }}</td>
            <td class="px-4 py-3 text-muted">{{ appt.hora_inicio?.slice(0, 5) }}</td>
            <td class="px-4 py-3 font-bold text-ink">{{ appt.client.user.name ?? '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ appt.service.nombre ?? '—' }}</td>
            <td class="px-4 py-3 text-muted">{{ appt.barber.user.name ?? '—' }}</td>
            <td class="px-4 py-3 text-center">
              <span class="rounded-full border px-2 py-0.5 text-[10px] font-black" :class="ESTADO_CLASS[appt.estado]">{{ ESTADO_LABEL[appt.estado] ?? appt.estado }}</span>
            </td>
            <td class="px-4 py-3">
              <div class="flex justify-end gap-2">
                <button type="button" class="rounded-lg border border-line px-3 py-1 text-xs text-muted hover:text-ink" @click="openEdit(appt)">Editar</button>
                <button
                  v-if="appt.estado !== 'cancelada' && appt.estado !== 'completada'"
                  type="button"
          ...
          <tr v-if="!appointments.length">
            <td colspan="7" class="px-4 py-12 text-center text-sm text-muted">Sin citas que mostrar.</td>
          </tr>
        </tbody>
      </table>
    </section>
    ...
    <div v-if="showForm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" @click.self="showForm = false">
      <div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-line bg-card p-6">
        <h2 class="mb-4 text-lg font-semibold text-ink">{{ editing ? 'Editar cita' : 'Nueva cita' }}</h2>
        <form class="space-y-3" @submit.prevent="submitForm">
          <div>
            <label class="mb-1 block text-xs text-muted">Cliente</label>
            <div v-if="form.client_label" class="flex items-center justify-between rounded-lg border border-line bg-main px-3 py-2 text-sm text-ink">
              <span>{{ form.client_label }}</span>
              <button type="button" class="text-xs text-muted hover:text-ink" @click="form.client_id = ''; form.client_label = ''">Cambiar</button>
            </div>
            <template v-else>
              <input v-model="clientQuery" type="text" placeholder="Buscar por nombre o email…" class="w-full rounded-lg border border-line bg-main px-3 py-2 text-sm text-ink">
              <ul v-if="clientOptions.length" class="mt-1 max-h-40 overflow-y-auto rounded-lg border border-line bg-main">
                <li v-for="c in clientOptions" :key="c.id">
                  <button type="button" class="block w-full px-3 py-2 text-left text-sm text-ink hover:bg-accent" @click="pickClient(c)">
          ...
          <div>
            ...
            <select v-model="form.estado" class="w-full rounded-lg border border-line bg-main px-3 py-2 text-sm text-ink">
              <option v-for="e in ESTADOS" :key="e" :value="e">{{ ESTADO_LABEL[e] }}</option>
            </select>
          </div>
          <div>
          ...
          <div class="mt-5 flex gap-3">
            <button type="submit" :disabled="saving || !form.client_id" class="flex-1 rounded-lg bg-gold px-4 py-2 text-sm font-semibold text-black hover:bg-gold-dim disabled:opacity-50">
