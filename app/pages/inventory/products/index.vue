...
async function markOrdered(item: LowStockItem) {
  ...
  try {
    ...
    actionNotice.value =
      dataErr?.message ?? "No se pudo marcar el producto como pedido.";
  } finally {
    marking.value = null;
  }
}
...
async function submitForm() {
  ...
  try {
    ...
    formError.value =
      dataErr?.message ?? "No se pudo guardar el producto. Verifica los datos.";
  } finally {
    saving.value = false;
  }
}
...
<template>
  <div class="p-4 sm:p-6 lg:p-8">
    ...
    >
      <div>
        ...
        <h1 class="mt-1 text-2xl font-semibold text-ink">
          Gestión de <span class="text-gold">Inventario</span>
        </h1>
        <p class="mt-1 text-sm text-muted">
          Control de stock, productos y suministros.
        </p>
      </div>
    ...
    <div
      v-if="actionNotice"
      class="mb-5 rounded-xl border border-gold/30 bg-gold/10 p-4 text-xs font-semibold text-gold"
    >
      {{ actionNotice }}
    </div>
        ...
        >
          <div>
            ...
            <p class="text-[10px] text-muted">
              {{ item.stock_actual }} en existencia / mín
              {{ item.stock_minimo }}
            </p>
          </div>
          <span
    ...
    <section v-if="stats" class="mb-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
      ...
      <div class="ui-card p-4">
        ...
        <p class="mt-2 text-2xl font-black text-emerald-400">
          {{ fmtMoney(stats.valor_inventario) }}
        </p>
      </div>
      <div class="ui-card p-4">
        <p class="text-xs font-semibold uppercase tracking-wider text-muted">
        ...
        <form class="space-y-4" @submit.prevent="submitForm">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              >
                {{ fieldErrors.precio_compra[0] }}
              </p>
            </div>
            <div>
              <label
                for="prod-precio-venta"
                class="mb-1 block text-xs font-semibold uppercase tracking-wider text-muted"
                >Precio venta ($)</label
              >
              <input
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              <label
                for="prod-stock-minimo"
                class="mb-1 block text-xs font-semibold uppercase tracking-wider text-muted"
                >Stock mínimo de alerta</label
              >
              <input
