...
interface OrdersResponse {
  ...
  meta: {
    ...
    last_page: number;
    total: number;
    stats?: { pendientes: number; entregados: number; por_cobrar: number };
  };
}
...
const {
  ...
  error,
  refresh,
} = await useAsyncData<OrdersResponse>(
  "orders-list",
  () =>
...
async function confirmDeliver() {
  ...
  actionError.value = "";
  try {
    await apiFetch(`/orders/${delivering.value.id}/deliver`, {
      method: "PATCH",
      body: { metodo_pago: metodoPago.value },
    });
...
async function downloadReceipt(order: OrderRow) {
  try {
    const blob = await apiFetch<Blob>(`/orders/${order.id}/receipt`, {
      responseType: "blob",
    });
  ...
  <div class="p-4 sm:p-6 lg:p-8">
    <header class="mb-5">
      ...
      <h1 class="mt-1 text-2xl font-semibold text-ink">
        Bandeja de <span class="text-gold">Pedidos</span>
      </h1>
      <p class="mt-1 text-sm text-muted">
        Gestión de pedidos de la tienda: entrega, cobro y cancelación.
      </p>
    </header>
    ...
    <section class="mb-5 flex flex-wrap items-center gap-3">
      ...
      >
        ...
        <option v-for="(label, val) in ESTADO_LABEL" :key="val" :value="val">
          {{ label }}
        </option>
      </select>
      <button
        v-if="search || estadoFilter"
        type="button"
        ...
        <tbody>
          ...
          >
            ...
            <td class="px-4 py-3 text-muted">
              {{ order.items.reduce((s, i) => s + i.cantidad, 0) }} pzas
            </td>
            <td class="px-4 py-3">
              <span
                class="rounded-full border px-2 py-0.5 text-[9px] font-black uppercase"
                :class="ESTADO_CLASS[order.estado]"
                >{{ ESTADO_LABEL[order.estado] ?? order.estado }}</span
              >
              <span v-if="order.metodo_pago" class="ml-1 text-[9px] text-muted"
                >({{
                  METODO_LABEL[order.metodo_pago] ?? order.metodo_pago
                }})</span
              >
            </td>
              ...
              <div class="flex justify-end gap-2">
                <template v-if="order.estado === 'pendiente'">
                  <button
                    ...
                    class="rounded-lg bg-emerald-500 px-3 py-1 text-xs font-bold text-black hover:bg-emerald-400 disabled:opacity-50"
                  ...
                  <button
                    type="button"
                    :disabled="busy === order.id"
                    class="rounded-lg border border-red-500/20 px-3 py-1 text-xs text-red-400 hover:bg-red-500/10 disabled:opacity-50"
                ...
                <button
                  v-else-if="order.estado === 'entregado'"
                  type="button"
