...
const TIER_CLASS: Record<string, string> = { nuevo: 'mc-nuevo', regular: 'mc-regular', vip: 'mc-vip', leyenda: 'mc-leyenda' }
const RANK: Record<string, number> = { nuevo: 0, regular: 1, vip: 2, leyenda: 3 }
...
<template>
  <div class="mc-stage">
    <div class="mc-tilt" :style="tiltStyle" @pointermove="onPointerMove" @pointerleave="onPointerLeave">
            ...
            <div class="mc-row">
              <div class="mc-brand">
                <svg viewBox="0 0 100 116" aria-hidden="true">
                  <path fill="none" stroke="#d4af37" stroke-width="4" d="M50 8 87 24v31c0 24-16 41-37 50C29 96 13 79 13 55V24L50 8Z" />
    ...
    <div class="mc-actions">
      ...
      <a v-if="downloadUrl" :href="downloadUrl" target="_blank" rel="noopener" class="mc-btn">Descargar tarjeta</a>
    </div>
  </div>
</template>
...
.mc-tilt { transform-style: preserve-3d; transition: transform 0.12s ease; }
.mc-flip {
  position: relative;
  width: 100%;
...
.mc-face {
  ...
  box-shadow: 0 24px 55px rgba(0, 0, 0, 0.6), 0 2px 0 rgba(255, 255, 255, 0.05) inset;
...
.mc-glare {
  ...
  background: radial-gradient(220px circle at var(--mx, 50%) var(--my, 50%), rgba(255, 255, 255, 0.28), transparent 60%);
}
...
.mc-wm { font-weight: 900; text-transform: uppercase; letter-spacing: -0.02em; font-size: 15px; color: #f4f4f2; }
...
.mc-chip {
  ...
  box-shadow: 0 1px 2px rgba(0, 0, 0, 0.4) inset, 0 1px 1px rgba(255, 255, 255, 0.3);
}
.mc-chip::after {
  content: '';
  position: absolute;
  ...
  border-radius: 3px;
  background:
    linear-gradient(90deg, transparent 46%, rgba(0, 0, 0, 0.35) 46%, rgba(0, 0, 0, 0.35) 54%, transparent 54%),
    linear-gradient(0deg, transparent 44%, rgba(0, 0, 0, 0.35) 44%, rgba(0, 0, 0, 0.35) 56%, transparent 56%);
}
.mc-foot { margin-top: auto; }
.mc-holder { font-size: 17px; font-weight: 800; letter-spacing: 0.02em; text-transform: uppercase; margin-top: 12px; color: #fff; line-height: 1.15; }
.mc-num { font-family: ui-monospace, 'Roboto Mono', Menlo, Consolas, monospace; font-size: 12px; letter-spacing: 0.16em; color: rgba(255, 255, 255, 0.72); margin-top: 3px; }
.mc-meta { display: flex; justify-content: space-between; align-items: flex-end; margin-top: 11px; }
.mc-lab { font-size: 7.5px; font-weight: 800; letter-spacing: 0.18em; text-transform: uppercase; color: rgba(255, 255, 255, 0.5); }
...
.mc-back-hint { font-size: 8px; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase; color: rgba(255, 255, 255, 0.45); margin-top: 4px; }
...
.mc-vip {
  ...
  box-shadow: 0 24px 55px rgba(0, 0, 0, 0.6), 0 0 0 1px rgba(212, 175, 55, 0.25) inset, 0 2px 0 rgba(255, 255, 255, 0.06) inset;
}
...
.mc-btn:hover { border-color: rgba(212, 175, 55, 0.6); background: rgba(212, 175, 55, 0.1); }
@media (prefers-reduced-motion: reduce) {
  .mc-sweep.mc-front::before, .mc-holo, .mc-flip { animation: none; transition: none; }
}
</style>
