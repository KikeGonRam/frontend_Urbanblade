...
const levelColor = computed(
  () => LEVEL_COLOR[props.data.loyalty.nivel] ?? "#d4af37",
);
...
const summaryCards = computed(() => {
  ...
  return [
    ...
    {
      label: "Puntos",
      value: Number(props.data.loyalty.puntos).toLocaleString(),
      caption: props.data.loyalty.nextNivel
        ? `Meta: ${props.data.loyalty.nextNivelLabel}`
        : "Nivel máximo",
      accent: LEVEL_COLOR[props.data.loyalty.nivel] ?? "#d4af37",
...
const visitChartOptions = {
  ...
  scales: {
    y: {
      ...chartScale(),
      beginAtZero: true,
      ticks: { ...chartScale().ticks, stepSize: 1 },
    },
    x: chartScale(),
  },
};
...
<template>
  <div class="space-y-5">
    ...
    >
      <div
        class="pointer-events-none absolute -top-24 -right-24 h-72 w-72 rounded-full bg-gold/10 blur-3xl"
      />
      <div
        class="pointer-events-none absolute bottom-0 -left-24 h-56 w-56 rounded-full bg-sky-500/10 blur-3xl"
      />
      <div class="relative grid gap-6 xl:grid-cols-[1fr_360px] xl:items-center">
        <div class="flex flex-col gap-5 sm:flex-row sm:items-center">
          ...
          <div class="min-w-0">
            <p
              class="text-[9px] font-black uppercase tracking-[0.32em] text-gold/70"
            >
              Tu panel personal
            </p>
            <h3
              class="mt-1 text-2xl font-black uppercase leading-tight text-ink sm:text-3xl"
            >
              Hola, <span class="text-gold">{{ firstName }}</span>
            </h3>
            <p class="mt-2 max-w-2xl text-sm text-ink/50">
              Reserva, revisa tu próxima visita, consulta tus puntos y encuentra
              productos recomendados sin perderte entre pantallas.
            </p>
          </div>
        </div>
    ...
    <section class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <article
        ...
        :key="card.label"
        class="relative overflow-hidden rounded-2xl border border-ink/[0.08] bg-card p-5"
      >
        <div
        ...
        <div class="mb-4 flex items-start justify-between gap-3">
          <div>
            <p
              class="text-[9px] font-black uppercase tracking-[0.25em] text-ink/40"
            >
              {{ card.label }}
            </p>
    ...
    <section class="grid grid-cols-1 gap-5 xl:grid-cols-12">
      <article
        ...
        :class="
          data.nextAppointment
            ? 'border-gold/25'
            : 'border-dashed border-gold/20'
        "
      >
        <div
          class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(212,175,55,0.13),transparent_36%)]"
        />
        <div
          v-if="data.nextAppointment"
          class="relative grid gap-0 md:grid-cols-[180px_1fr]"
        >
          <div
          ...
          >
            <div class="text-center">
              <p
                class="text-[8px] font-black uppercase tracking-[0.22em] opacity-60"
              >
                Próxima cita
              </p>
              ...
              <p class="text-xs font-black uppercase opacity-70">
                {{ data.nextAppointment.monthShort }}
              </p>
            </div>
            <div class="h-10 w-px bg-black/15 md:h-px md:w-20" />
            <p class="text-2xl font-black">
              {{ data.nextAppointment.hora_inicio?.slice(0, 5) }}
            </p>
          </div>
          <div class="p-6">
            <div
            ...
            >
              <div class="min-w-0">
                <p
                  class="text-[9px] font-black uppercase tracking-[0.25em] text-gold/70"
                >
                  Agenda confirmable
                </p>
                <h3
                  class="mt-2 text-2xl font-black uppercase leading-tight text-ink"
                >
                ...
                <p class="mt-1 text-sm font-bold text-ink/45">
                  ...
                  <span class="text-gold">{{
                    data.nextAppointment.barber?.user?.name ??
                    "Maestro UrbanBlade"
                  }}</span>
                </p>
                <div class="mt-4 flex flex-wrap gap-2">
                  <span
                    ...
                    >{{ data.nextAppointment.dateLong }}</span
                  >
                  <span
                    ...
                    >{{ data.nextAppointment.estado ?? "pendiente" }}</span
                  >
                </div>
              </div>
            </div>
        ...
        >
          <div>
            <p
              class="text-[9px] font-black uppercase tracking-[0.28em] text-gold/70"
            >
              Agenda libre
            </p>
            <h3 class="mt-1 text-xl font-black uppercase text-ink">
              Sin citas próximas
            </h3>
      ...
      >
        ...
        <div class="relative flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            <p
              class="text-[9px] font-black uppercase tracking-widest text-sky-300/80"
            >
              Sugerencia inteligente
            </p>
            <p class="mt-1 text-xl font-black leading-tight text-ink">
              {{ data.recommendation.valorDestacado }}
            </p>
      ...
      <article
        v-else
        class="rounded-2xl border border-ink/[0.08] bg-card p-6 xl:col-span-5"
      >
        <p class="text-[9px] font-black uppercase tracking-widest text-ink/40">
          Recomendaciones
        </p>
        <p class="mt-2 text-lg font-black text-ink">
          Aún no hay patrón suficiente
        </p>
        <p class="mt-2 text-xs leading-relaxed text-ink/45">
          Cuando tengas más visitas, el sistema podrá sugerirte servicios y
          productos acordes a tu historial.
        </p>
      </article>
    </section>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-12">
      <div
        class="rounded-2xl border border-ink/[0.08] bg-card p-5 lg:col-span-7"
      >
        <div
        ...
        >
          <div>
            ...
            <h3 class="mt-0.5 text-lg font-black uppercase text-ink">
              Ritmo de visitas
            </h3>
          ...
          <div class="grid grid-cols-2 gap-2 text-right sm:min-w-56">
            ...
            >
              <p
                class="text-[8px] font-black uppercase tracking-widest text-ink/35"
              >
                Periodo
              </p>
              <p class="mt-0.5 text-sm font-black text-gold">
                {{ visitTotal6 }} visitas
              </p>
            </div>
            <div
        ...
        <div
          v-else
          class="flex h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-ink/[0.08] bg-ink/[0.02] text-center"
        >
          <p class="text-xs font-black uppercase tracking-widest text-ink/45">
            Sin historial aún
          </p>
          <p class="mt-2 max-w-xs text-xs text-ink/35">
            Después de tus primeras visitas, aquí aparecerá tu tendencia
            mensual.
          </p>
        </div>
      </div>
        ...
        <div class="space-y-4 rounded-2xl border border-ink/[0.08] bg-card p-5">
          ...
          >
            <div
              ...
              :style="{
                background: `conic-gradient(${levelColor} ${Math.round(safeProgress)}%, ${inkRgba(0.08)} 0)`,
              }"
            >
              <div
                class="grid h-14 w-14 place-items-center rounded-full bg-card"
              >
                <span class="text-sm font-black text-ink"
                  >{{ Math.round(safeProgress) }}%</span
                >
              </div>
            </div>
            <div>
              <p
                class="text-[8px] font-black uppercase tracking-[0.22em] text-ink/40"
              >
                Lealtad
              </p>
              <p class="mt-1 text-lg font-black uppercase text-ink">
                {{ data.loyalty.nivelLabel }}
              </p>
              <p
                v-if="data.loyalty.nextNivel"
                class="mt-1 text-xs font-bold text-ink/45"
              >
                Próximo nivel:
                <span class="text-gold">{{ data.loyalty.nextNivelLabel }}</span>
                ·
                {{
                  data.loyalty.citasFaltan > 0
                    ? `faltan ${data.loyalty.citasFaltan} visita${data.loyalty.citasFaltan !== 1 ? "s" : ""}`
                    : "listo para subir"
                }}
              </p>
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            >
              ...
              <span class="text-[9px] font-bold leading-tight">{{
                ben.label
              }}</span>
            </div>
          </div>
          ...
          <div v-if="data.loyalty.recentTransactions.length">
            <p
              class="mb-2 text-[8px] font-black uppercase tracking-[0.22em] text-ink/45"
            >
              Últimos movimientos
            </p>
                    ...
                    <path
                      ...
                      :d="
                        tx.puntos > 0
                          ...
                          : 'M12 5v14m0 0l7-7m-7 7l-7-7'
                      "
                    />
                  </svg>
                  <span class="text-[9px] text-ink/50">{{
                    tx.descripcion
                  }}</span>
                </div>
                <span
          ...
          >
            ...
            <div>
              <p
                ...
                style="color: rgba(232, 121, 249, 0.8)"
              >
                Ganaste el sorteo de {{ data.loyalty.wonRaffle.mes }}
              </p>
              <p class="text-[8px] text-ink/50">
                {{ data.loyalty.wonRaffle.premio }}
              </p>
              <p
                v-if="data.loyalty.wonRaffle.isExpired"
                class="mt-0.5 text-[8px] text-red-400/70"
              >
                Caducó el {{ data.loyalty.wonRaffle.venceEn }} sin reclamarse
              </p>
              <p v-else class="mt-0.5 text-[8px] text-emerald-400/70">
                Válido hasta {{ data.loyalty.wonRaffle.venceEn }} — coméntalo en
                tu próxima cita
              </p>
            </div>
          </div>
