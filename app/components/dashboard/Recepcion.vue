...
const kpiCards = computed(() => [
  ...
  {
    label: "Cobrado Hoy",
    val: `$${Number(props.data.kpis.collected_today ?? 0).toLocaleString("es-MX", { maximumFractionDigits: 0 })}`,
    text: "text-emerald-400",
    line: "from-emerald-500/60",
  },
  {
    label: "Cobros Pend.",
    val: props.data.kpis.pending_payments,
    text: "text-amber-400",
    line: "from-amber-500/60",
  },
  {
    label: "Pedidos",
    val: props.data.kpis.pending_orders ?? 0,
    text: "text-cyan-400",
    line: "from-cyan-500/60",
  },
  {
    label: "Nuevos Clientes",
    val: props.data.kpis.new_clients_today,
    text: "text-indigo-400",
    line: "from-indigo-500/60",
  },
  {
    label: "Stock Crítico",
    val: props.data.kpis.low_stock_count,
    text: "text-red-400",
...
<template>
  <div class="space-y-5">
    ...
    >
      <div
        class="absolute -bottom-8 -right-8 h-32 w-32 rounded-full bg-indigo-500/5 blur-2xl"
      />
    ...
    <section class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
      ...
      >
        <div
          class="absolute left-0 top-0 h-0.5 w-full bg-gradient-to-r to-transparent"
          :class="kpi.line"
        />
        <p
          class="mb-3 text-[9px] font-black uppercase tracking-[0.2em] text-ink/50"
        >
          {{ kpi.label }}
        </p>
    ...
    <section class="grid grid-cols-1 gap-5 lg:grid-cols-12">
      ...
      >
        ...
        <div class="space-y-2">
          ...
          >
            <div
              class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-indigo-500/10 text-xs font-black text-indigo-400"
            >
              {{ appt.hora_inicio?.slice(0, 2) }}
            </div>
            <div class="min-w-0 flex-1">
              <p class="truncate text-xs font-black uppercase text-ink">
                {{ appt.cliente }}
              </p>
            ...
            <p class="shrink-0 text-xs font-black text-ink">
              {{ appt.hora_inicio?.slice(0, 5) }}
            </p>
          </div>
          <div
            v-if="!data.nextAppointments.length"
            class="flex items-center justify-center rounded-xl border border-dashed border-ink/[0.06] py-12"
          >
    ...
    <section class="rounded-2xl border border-ink/[0.06] bg-card p-5">
      ...
      >
        ...
        <p class="shrink-0 text-sm font-black text-gold">
          ${{ order.total.toFixed(2) }}
        </p>
      </div>
      <div
        v-if="!data.pendingOrders.length"
        class="flex items-center justify-center rounded-xl border border-dashed border-ink/[0.06] py-12"
      >
        <p class="text-xs font-bold uppercase tracking-widest text-ink/45">
